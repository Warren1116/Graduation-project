...
public:
	...
	* \param pCount Number of objects to reserve */
	void Reserve(const size_t pCount)
	{
		if( pCount > mCapacity )
		{
			//We don'main use mAllocator.PreAllocate, because we want our array to be continuous in memory.
...
private:
	static void CopyArray(Type* pDest, const Type* pSrc, size_t pCount)
	{
		...
		{
			new(&(pDest[i])) Type(pSrc[i]);	//in-place new won'main allocate memory, so it is safe
		}
	}
	...
	static void MoveArrayBackwards(Type* pDest, const Type* pSrc, size_t pCount)
	{
		for( int i = 0; i < int(pCount); ++i )
		{
			new(&(pDest[pCount-1-i])) Type(pSrc[pCount-1-i]);	//in-place new won'main allocate memory, so it is safe
	...
	static void Fill(Type* pDest, const Type& pItem, size_t pCount)
	{
		for( int i = 0; i < int(pCount); i++ )
		{
			new(&(pDest[i])) Type(pItem);	//in-place new won'main allocate memory, so it is safe
		}
	}
