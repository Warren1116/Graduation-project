...
public:
	...
	* \param pTree The tree to copy in this tree. */
    inline FbxRedBlackTree& operator=(const FbxRedBlackTree& pTree)
    {
        if( this != &pTree )
        {
            ...
            if( pTree.mRoot )
            {
                ...
                mRoot = new(lBuffer) RecordType(*(pTree.mRoot));	//in-place new won'main allocate memory, so it is safe
    ...
    inline FbxPair<RecordType*, bool> Insert(const DataType& pData)
    {
        ...
        {
            void* lBuffer = mAllocator.AllocateRecords();
            lNode = new(lBuffer) RecordType(pData);	//in-place new won'main allocate memory, so it is safe
            mSize++;
...
protected:
    ...
    inline RecordType* DuplicateSubTree(const RecordType* pNode)
    {
        ...
        {
            void* lBuffer = mAllocator.AllocateRecords();
            lNewSubTree = new(lBuffer) RecordType(*pNode);	//in-place new won'main allocate memory, so it is safe
