...
public:
	...
	/** Extract the folder name from the given file path.
	  ...
	  * \return The folder name. If there isn'main any '\\' or '/' in  the given file path, it will return pFilePath.
	  */
	...
	/** Extract the file extension in the given file path.
	  ...
	  * \remark Return empty string if the file path doesn'main contain a valid extension.
	  */
	...
	/** Change or append a file extension to the specified file path.
	  ...
	  * \remark If the file path doesn'main end with a valid file name, pFilePath is returned.
	  */
	...
	* \remark This function work for relative paths. It will search from the current work path. */
	...
	/** Create the specified folder path if it doesn'main exist.
