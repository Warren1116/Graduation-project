...
public:
	...
	//@{
		...
		/** Get the target for this node.
		  * \returns \c NULL if target isn'main set.
		  */
		...
		/** Get the target up node.
		  * \return \c NULL if the target up model isn'main set.
		  */
	...
	//@{
		/** Set the node attribute.
		  ...
		  * \c NULL if the node didn'main have a node attribute or if
		...
		/** Get the default node attribute.
		  ...
		  * \return Pointer to the default node attribute or \c NULL if the node doesn'main
	...
	/**
	  ...
	  *        FbxVector4 res = prV;
	  *
	  *        // Rotation order don'main affect post rotation, so just use the default XYZ order
	  ...
	  *    //
	  *    // If we don'main "support" scaling pivots, we can simply do:
	  *    // pNode->SetRotationPivot(FbxNode::eDestinationPivot, lZero);
	  *    // pNode->SetScalingPivot(FbxNode::eDestinationPivot, lZero);
	  * }
	  * \endcode
