...
void Sprite::Render(ID3D11DeviceContext* immediate_context, float dx, float dy, float dw, float dh, float sx, float sy, float sw, float sh, float angle, Pivot pivot, float r, float g, float b, float a) const
{
	{
		// 現在設定されているビューポートからスクリーンサイズを取得する。
		D3D11_VIEWPORT viewport;
...
void Sprite::RenderNumber(ID3D11DeviceContext* immediate_context, int nums, float x, float y, float w, float h, float r, float g, float b, float a)
{
...
void Sprite::Update(float dx, float dy, float dw, float dh, float sx, float sy, float sw, float sh, float angle, float r, float g, float b, float a) const
{
